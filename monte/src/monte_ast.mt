...
def astWrapper(node, maker, args, span, transformArgs, termFunctor):
    return object astNode extends node:
        to asTerm():
            return term`$termFunctor(*${transformArgs(fn x {x.asTerm()})})`.withSpan(span)
        to transform(f):
            return f(node, maker, transformArgs(f), span)
        to _uncall():
